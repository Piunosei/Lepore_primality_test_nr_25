...
int main(){
  ...
  mpz_t N,p,q,mom1,mom2,mom3,M,X,a,zero,uno,due,exp,b,M2,y,z,r,A,B,resto,copia_di_exp,TEST,max_a,venticinque;
...
if(mpz_cmp(N,p)>=0){
   ...
   if(/*mpz_cmp(p,uno)!=0 && */  (mpz_cmp(p,N)!=0 &&  mpz_cmp(mom3,zero)==0) || mpz_cmp(b,venticinque)>=0){
   break;
 }
 mpz_set(p,uno);////////////////////////////////////////
 mpz_add(b,b,uno);
